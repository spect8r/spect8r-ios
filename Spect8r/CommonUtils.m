...
- (void) setRoundedRectBorderView:(UIView *)imageView withBorderWidth:(float) width withBorderColor:(UIColor *) color withBorderRadius:(float)radius {
    imageView.clipsToBounds = YES;
    imageView.layer.cornerRadius = radius;
...
-(NSString*)encodeMediaPathURLToBase64String:(NSURL*)mediaPath
{
    NSData *data = [NSData dataWithContentsOfURL:mediaPath];
    
    NSString *media = [NSString base64StringFromData:data length:data.length];
...
//    [[UITabBar appearance] setBarTintColor:appController.appTextColor];     // Bar background
//    [[UITabBar appearance] setSelectedImageTintColor:RGBA(255, 255, 255, 1)];   //bar item selected color
    
//    [[UITabBar appearance] setShadowImage:nil] ;
    
//    [[UITabBarItem appearance] setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:RGBA(246, 134, 31, 1), NSForegroundColorAttributeName, nil] forState:UIControlStateHighlighted];
    
    int tabBarItemsCount = 4;
...
/*    UIColor *invertColor = [UIColor blackColor];
    [button setBackgroundColor:state ? appController.appMainColor : invertColor];
    [button setTitleColor:state ? invertColor : appController.appMainColor forState:UIControlStateNormal];
...
- (NSString *)getLocalizedURL
{
/*    if ([[LanguageManager selectedLanguage] isEqualToString:@"ja"])
    {
        return @"jp";
    }
    else
...
#pragma mark UIView Gradient  ------- Black - White - Black
...
-(void)gradientView:(UIView*)view
{
    ...
    gradient.colors = [NSArray arrayWithObjects:(id)[RGBA(50, 50, 50, 1) CGColor], (id)[RGBA(170, 170, 170, 0.5) CGColor], (id)[RGBA(50, 50, 50, 1) CGColor], nil];
