...
//    self.navigationItem.leftBarButtonItem = self.navBarMenu;
    ...
    UIView *searchView = [[UIView alloc] initWithFrame:CGRectMake(self.width * 0.1f, self.height * 0.05f, self.width * 0.8f, self.height *0.1f)];
    [searchView setBackgroundColor:UIColorFromRGBAndAlpha(0xAAAAAA, 0.5f)];
    [searchView.layer setCornerRadius:0];
    [searchView.layer setBorderWidth:1];
    [searchView.layer setBorderColor:[UIColorFromRGB(0xAAAAAA) CGColor]];
    
    UIImageView *imvSearch = [[UIImageView alloc] initWithFrame:CGRectMake(searchView.bounds.size.width * 0.05f, searchView.bounds.size.height / 2 - 16, 32, 32)];
    [imvSearch setImage:[UIImage imageNamed:@"icon_search"]];
    [searchView addSubview:imvSearch];
    
    
    self.txtSearch = [[UITextField alloc] initWithFrame:CGRectMake(searchView.bounds.size.width * 0.2f, searchView.bounds.size.height / 2 - 15, searchView.bounds.size.width * 0.8f, 30)];
    self.txtSearch.font = MuseosansFont(16);
    [self.txtSearch setAttributedPlaceholder:[[NSAttributedString alloc] initWithString:@"search" attributes:@{NSForegroundColorAttributeName:UIColorFromRGB(0xAAAAAA)}]];
    ...
    NSString *string1 = @"Search for friends, artists, or venues";
    ...
    UILabel *lbltitle = [[UILabel alloc] initWithFrame:CGRectMake(self.width * 0.1f, self.height * 0.2f - 15, self.width * 0.8f, 30)];
    [lbltitle setAttributedText:attributedString1];
    [lbltitle setTextColor: [UIColor whiteColor]];
    [lbltitle setTextAlignment:NSTextAlignmentCenter];
    [self.view addSubview:lbltitle];
    
    UIView *line1 = [[UIView alloc] initWithFrame:CGRectMake( 0, self.height * 0.25f, self.width, 3)];
